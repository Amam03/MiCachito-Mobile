...
**Estado actual (2026-09-11, post 384b1c7 + Expendios commiteado 3eafa38):** Fase SOLO INTERFAZ con mock data — la app NO está conectada al backend salvo Login (funciona contra el backend real y persiste sesión). Implementado y pusheado: pestaña Vender LOTENAL completa + Sorteos Tec + Consulta de Premios (escáner QR integrado); pestaña Gestión completa: Tickets de Venta, Devolución, Premios y Reintegros, Sorteos, Depósitos, Recibos de Pago, Reportes (Estado de Cuenta + Fondo de Ahorro + Facturación) y Tira de Liquidación (deshabilitada); Tiempo de Aire implementado desde agosto (pendiente revisar contra mockups frescos); **Expendios 1-6 IMPLEMENTADO Y COMMITEADO (3eafa38):** pantalla principal con 2 pestañas (Consulta de registros con modal fechas/expendios + Administración con tarjetas y FAB) + form Actualizar/Crear compartido con Permisos de Venta — ver `docs/NOTAS_EXPENDIOS.md`. **Cuenta 1-6 IMPLEMENTADO (2026-09-11, sin commit):** pestaña final con identidad de la app (logo+nombre+versión real), usuario autenticado real, Notificaciones (estado vacío), Impresora desplegable con dispositivos BT vinculados reales (`IImpresoraService.ObtenerDispositivosEnlazadosAsync` nuevo), diálogo Atención sin impresora (solo 1a vez), Dispositivos Enlazados, y Cerrar Sesión con modal + logout real + fix de plataforma del back press (manifest opt-out back predictivo + MainActivity.OnBackPressed → MoveTaskToBack en raíz; crash dotnet/maui#32750) — ver `docs/NOTAS_CUENTA.md`. Las 4 pestañas del menú principal están completas. Reglas de estilo fijadas en `docs/Convenciones.md` §4.5-4.6 (BoxView/Padding, cero emojis, NavBarIsVisible, fechas es-MX). Documentación por módulo en `docs/NOTAS_*.md`.
